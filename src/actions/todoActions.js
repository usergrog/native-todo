...
function updateTodo(todo, state) {
  ...
  return fire
    .database()
    .ref("todos/" + selectedGroup.id + "/" + todo.id)
    .set(todo);
}
  ...
  return (dispatch, getState) => {
    ...
    fire
      ...
      .then(t => {
        ...
        const updatedGroups = [
          ...groups.slice(0, updatedIndex),
          group,
          ...groups.slice(updatedIndex + 1)
        ];
...
export function changePriority(todoSource, val) {
  return (dispatch, getState) => {
    dispatch(showProgress());
    let todos = getState().todoReducer.todos;
    // set new position
    let todoTargetOrderVal = todoSource.orderVal + val;

    todos.forEach(todo => {
      if (todoTargetOrderVal === todo.orderVal) {
        todo.orderVal = todo.orderVal - val;
        updateTodo(todo, getState());
      }
    });

    todoSource.orderVal = todoTargetOrderVal;
